...
public class SiangMalam : MonoBehaviour {
	...
	void UpdateBGColor(){
		if (siang == true) {
			if (sr1.material.GetFloat ("_Val") >= 0.25f) {
				sr1.material.SetFloat ("_Val", sr1.material.GetFloat ("_Val") - 0.002f);
				sr2.material.SetFloat ("_Val", sr2.material.GetFloat ("_Val") - 0.002f);


				if (statusBulan > -2 && statusBulan<7 &&sr1.material.GetFloat ("_Val") <= 0.95f) {
	...
	public void UbahXBulan(){
		...
		if (statusBulan == -1) {
			bulanPurnama = false;
			bulan.GetComponent<SpriteRenderer> ().flipX = false;
		}
